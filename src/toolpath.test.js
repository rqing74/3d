...
import {createToolpath,layerCount,advanceProgress,BED_Y,WORLD_HEIGHT} from './toolpath.js';
  ...
  test(`${model}: deposition stays inside the bed and rises continuously`,()=>{
    ...
    for(let i=0;i<points.length;i++) {
      ...
      if(i>0) {
        const p=points[i-1];
        assert.ok(y>=p[1]);
        assert.ok(Math.hypot(x-p[0],y-p[1],z-p[2])<0.12,'No disconnected extrusion between layers');
      }
    }
  });
}
