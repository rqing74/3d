...
export function layerCount(height) { return Math.round(MODEL_HEIGHT_MM / height); }
...
export function perimeterPoint(model, t, angle) {
  ...
  if (model === 'cube') {
    ...
    const side = Math.floor(a), f = a - side;
    [x,z] = [[-0.95 + 1.9*f, -0.95], [0.95, -0.95 + 1.9*f], [0.95 - 1.9*f, 0.95], [-0.95, 0.95 - 1.9*f]][side];
  } else if (model === 'twist') {
    const radius = 0.86 + 0.11 * Math.cos(4 * angle);
    const a = angle + t * Math.PI * 0.85;
    x = Math.cos(a)*radius; z = Math.sin(a)*radius;
  } else {
    const belly = 0.72 + 0.34*Math.sin(t*Math.PI*1.65) + 0.20*Math.pow(t,8);
...
export function createToolpath(model, layers) {
  ...
  for (let i=0; i<=layers*STEPS; i++) {
    points.push(perimeterPoint(model, i/(layers*STEPS), i/STEPS*Math.PI*2));
  }
  return points;
}
...
export function advanceProgress(progress, delta, printSpeed, simulationSpeed) {
  ...
  return Math.min(1, progress + Math.max(0, delta)/240 * (printSpeed/60) * simulationSpeed);
}
